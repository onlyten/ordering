...
//      每页显示的记录数
    ...
    public function getPrice()
	{
		...
		$price = D("Menu")->where(array("id" => $menuId))->getField("price");
        $this->ajaxReturn(
            array(
                "price" => $price,
    ...
    public function updateShop()
    {
	      ...
	      $shoplist = D("Shop")->where(array("id" => array("in", I("get.id"))))->field('id,name,address,bank,bank_card')->select();
	      
	      foreach ($shoplist as $k => $val) {
             $url=CONNECT."MerchantAccount/".$shoplist[$k]['id']."/".urlencode($shoplist[$k]['name'])."/".urlencode($shoplist[$k]["address"])."/".urlencode($shoplist[$k]["bank"])."/".$shoplist[$k]["bank_card"];
             //echo $url;
             ...
             if($result[0]->success == '1'){     
             	   //echo "成功";   exit;         
                 M("Shop")->where('id='.$shoplist[$k]['id'])->save(array("status" => '2'));
                 $this->success("审核成功", cookie("prevUrl"));
             }else{
                	//echo "失败"; 
            	   $this->error($result[0]->msg ,cookie("prevUrl"));                         
        	   }
        }	                        

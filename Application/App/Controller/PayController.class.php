...
class PayController extends BaseController
{
    ...
    public function wxNotify()
    {
        ...
        if ($notify->checkSign() == TRUE) {
			...
			$custId ="675";  //$order['user_id'];
			
			$orderList= D('Count')->where('orderId='.$out_trade_no)->field('productId,productName,orderDate')->order('orderDate  asc')->select();
			//查询总条数
			$count = D('Count')->where('orderId='.$out_trade_no)->field('productId,productName,orderDate')->count();
			if(!empty($orderList))
			{
				$beginDate = $orderList[0]['orderdate'];
				if(!empty($count))
				{
					$endDate = $orderList[$count-1]['orderdate'];
				}

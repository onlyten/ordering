...
class TradeController extends BaseController
{
	...
	public function statements()
	{
		...
		$url = CONNECT."MerchantFund/".session("homeShopId");
		
		$result = $this->_request($url,true,'POST');
		$result = json_decode($result,true);
		$this->assign("tradeList",$result);
		/*if(!empty($result[0]->merchantID)){
			$this->assign("money",$result[0]->payMoney);
			$this->assign("redMoney",$result[0]->resideMoney);
			$this->assign("merchantName",$result[0]->merchantName);
		}else
		{
			$this->error("系统异常");
		}
